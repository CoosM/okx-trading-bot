...
def sell_spot():
    ...
    sell_percent = 1 / step
    qty_raw = balance * sell_percent

    # Обрезаем вниз до 8 знаков (OKX поддерживает до 8 на споте)
    factor = 10 ** 8
    sell_qty = math.floor(qty_raw * factor) / factor

    if sell_qty <= 0:
        return {"SELL": "SKIP", "reason": "qty too small"}

    log(
        f"🔴 SELL TRY | balance={balance:.8f} | "
